export default {
    'register.page': '注册',
    'register.submit.text': '注册',
    'register.submit.loadingText': '注册中..',
    'validation.capthcha.secondText': '{s}秒',
    'validation.capthcha.sendSuccess': '验证码已发送，请注意查收短信，',
    'validation.capthcha.notGet': '收不到验证码？',
    'register.mobile.takeUp': '手机号已被占用，请更换手机号或去',

    'login.page': '登录',
    'login.page.oauth': '第三方登陆验证',
    'user.settings.profile': '个人信息',
    'user.settings.account': '账户管理',
    'user.settings.notifications': '消息管理',
    'user.dashboard.recent': '最近编辑',
    'user.dashboard.article': '我的文章',
    'user.dashboard.question': '我的提问',
    'user.dashboard.answer': '我的回答',
    'user.dashboard.column': '我的专栏',
    'user.dashboard.follows': '我的关注',
    'user.dashboard.wallet': '我的钱包',
    'user.dashboard.collections': '我的收藏',
    'user.dashboard.history': '最近浏览',
    'user.notifications': '通知中心',
};
