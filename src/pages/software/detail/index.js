...
import { Card, Space, Descriptions, Tabs , message, Empty , Collapse, Table, Tooltip, Popover } from 'antd';
...
const SoftwareDetail = ({ match: { params } }) => {
        ...
        return (
            <div className={styles['software-attributes']}>
                <Descriptions column={1} size="small" bordered>
                    {
                        attributes.map(({ id , name , value }) => <Descriptions.Item key={id} label={getAttributes(name)}>{ name === "website" ? <a target="_blank" href={value}>{value}</a> : value}</Descriptions.Item>)
                    }
    ...
    const renderColumns = () => [
        {
          ...
          render:(_,{ title , name , size , md5 , sha1 , sha256}) => {
            ...
            content={
            <>
            { name && <span><strong>文件：</strong>{name}<br/></span> }
            { size && <span><strong>大小：</strong>{filesize(size)}<br/></span> }
            { md5 && <span><strong>MD5：</strong>{md5}<br/></span> }
            { sha1 && <span><strong>SHA1：</strong>{sha1}<br/></span> }
            { sha256 && <span><strong>SHA256：</strong>{sha256}<br/></span> }
            </>
            }
            >
                {
        ...
        },
        {
          ...
          render:(_,{ name , ed2k , size }) => {
              ...
              return <Tooltip title={url}><a onClick={() => copy(url)}>复制</a></Tooltip>
          }
        },
        {
          title: 'BT',
    ...
    const renderReleases = () => {
        ...
        return <Tabs className={styles['software-release']} defaultActiveKey="-1">
            {
                releases.map(({ id , name , updaters }) => {
                    return <TabPane key={id} tab={name}>
                        <Collapse className={styles['software-updater']} ghost accordion defaultActiveKey={updaters && updaters.length > 0 ? updaters[0].id : 0 }>
                        {
                            updaters.map(({ id , name , files }) => {
                                return <Panel key={id} header={name}>
                                    <Table rowKey="id" dataSource={files} columns={renderColumns()} size="small" pagination={false} bordered />
                                </Panel>
                            })
                        }
                        </Collapse>
                            ...
                            <Card>
                                <div className={styles['header']}>
                                    <h2 className={styles['title']}>
                                        <Link to={`/software/${id}`} dangerouslySetInnerHTML={{ __html: name }} />
                                    </h2>
                                ...
                                {
                                    recommendFiles.length > 0 && <div className={styles['software-recommend']}>
                                    <h2 className={styles['title']}>推荐版本</h2>
                                    <Table rowKey="id" dataSource={recommendFiles} columns={renderColumns()} size="small" pagination={false} bordered />
                                    </div>
                                }
                                ...
                                <Space size="large">
                                    ...
                                    {!isMobile && !use_author && <ReportButton id={id} type="software" />}
                                </Space>
