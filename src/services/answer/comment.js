...
export async function getRoot({ question_id , answer_id , ...params}) {
    return request(`/api/question/${ question_id }/answer/${answer_id}/comment/root?${stringify(params)}`)
}

export async function getChild({ question_id , answer_id , id , ...params}) {

    return request(`/api/question/${ question_id }/answer/${answer_id}/comment/${id}/child${params ? "?" + stringify(params) : ""}`)
}
