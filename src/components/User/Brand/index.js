...
export default ({ id , placement , children }) => {
    ...
    const renderContent = () => {
        ...
        const { name , description , avatar , star_count , follower_count , profession , address , path } = detail
       
        const infoComponents = []
        if(profession && profession.trim() !== "") infoComponents.push(<p key="address">地址:{ address }</p>)
        if(address && address.trim() !== "") infoComponents.push(<p key="profession">行业:{ profession }</p>)
            ...
            <div className={styles['user-card']}>
                <div className={styles['body']}>
                    ...
                    <div className={styles['info']}>
                        <h2>{ name }</h2>
                        { description && description !== "" && <div className={styles['desc']}>{ description }</div> }
                        {
