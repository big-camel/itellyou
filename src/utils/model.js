export const setList = (key,{ append , end , total , data , ...payload },state,callback,megerBack) => {
    ...
    callback = callback || function(child,item){
        return child.id === item.id
    }
    total = list.total
    data.forEach(item => {
        const index = dataList.find(child => callback(child,item))
        if(index < 0){
            total += 1
            dataList.push(item)
        }else{
            const old = dataList[index]
            megerBack = megerBack || function(old,item){
                return {...old , ...item }
            }
...
export const replaceItem = (key , { ...payload } , state , callback , megerBack ) => {
    ...
    if(list){
        ...
        if(index >= 0){
            ...
            megerBack = megerBack || function(item){
                return {...item , ...payload }
            }
