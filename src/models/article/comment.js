...
export default {
    ...
    effects:{
        *create({ payload }, { call , put }){
            ...
            if(response && response.result){
                ...
                yield put({
                    type:"addItem",
                    payload:{ type:"list", detail:response.data , to:replyId}
                })
                yield put({
                    type:"addItem",
                    payload:{ type:"detail",detail:response.data ,to:replyId}
                })
            }
        ...
        *delete({ payload }, { call , put }){
            ...
            if(response && response.result){
                yield put({
                    type:"replaceItem",
                    payload:{ type:"list", detail:response.data }
                })
                yield put({
                    type:"replaceItem",
                    payload:{ type:"detail", detail:response.data }
                })
            }
            return response
        },
        *root({ payload }, { call , put }){
            ...
            if(response && response.result){
                yield put({
                    type:"setList",
                    payload:{ type:"list",data:response.data }
                })
            }
            return response
        },
    ...
    reducers:{
        setList(state,{ payload : { type , data } }){
            return {
                ...state,
                [type]:data
            }
        },
        addItem(state,{ payload : { type , detail , to } }){
            const list = state ? state[type] : null
            if(list){
        ...
        replaceItem(state,{ payload : { type , detail , ...payload} }){
            const list = state ? state[type] : null
            if(list){
        ...
        addChild(state,{ payload : { type , id , append , childs } }){
            const list = state ? state[type] : null
            if(list){
